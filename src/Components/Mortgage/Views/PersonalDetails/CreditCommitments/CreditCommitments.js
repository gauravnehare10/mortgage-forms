...
export default function CreditCommitments() {
  ...
  const shouldDisplay = () => {
    const applicant = formData?.creditCommitmentsData?.applicant;
    const partner = formData?.creditCommitmentsData?.partner;
    
    return (
      (applicant?.hasExCreditCommit === 'Yes' && applicant?.creditCommitments?.length > 0) ||
      (partner?.hasExCreditCommit === 'Yes' && partner?.creditCommitments?.length > 0)
    );
  };
